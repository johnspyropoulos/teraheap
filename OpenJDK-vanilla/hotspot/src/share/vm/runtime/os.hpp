...
class os: AllStatic {
 ...
 private:
  ...
  // Same as pd_uncommit_memory(), but advises the Linux kernel
  // to free uncommited physical pages.
  static bool my_pd_uncommit_memory(char* addrr, size_t bytes);
 ...
 public:
  ...
  // Same as uncommit_memory() but uses my_pd_uncommit_memory() instead and
  // it called by the young heap generation.
  static bool   younggen_uncommit_memory(char* addr, size_t bytes);

  // // Same as uncommit_memory() but uses my_pd_uncommit_memory() instead and.
  // // is called by the old heap generation.
  static bool   oldgen_uncommit_memory(char* addr, size_t bytes);
